...
export const TransactionHistory = () => {
    ...
    const fetchTransactions = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setPagination(pag);
          setIsNewUser(data.length === 0 && !pag?.total); // Updated check for new user
        }
      } catch (err) {
  ...
  const getTransactionDetails = (transaction) => {
    ...
    return {
      ...
      otherParty: isSender
        ? transaction.toUser || { firstname: "Unknown", lastname: "" }
        : transaction.fromUser || { firstname: "Unknown", lastname: "" },
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500">
          Make your first transaction to start building your history.
        </p>
      </div>
    );
  }
